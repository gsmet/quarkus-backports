...
public class GitHubService {
    ...
    private static final String PULL_REQUESTS_FOR_BACKPORT_LABEL_URL = "https://github.com/%s/issues?q=state%%3Aclosed%%20label%%3A%s";
    ...
    public Collection<PullRequest> getPullRequestsTargetingBranch(Milestone milestone) throws IOException {
        JsonObject response = graphQLClient.graphql(token, new JsonObject()
                .put("query",
                        Templates.listPullRequestsTargetingBranch(repository.fullName(), milestone.minorVersion()).render()));
        // Any errors?
        if (response.getJsonArray("errors") != null) {
            throw new IOException(response.toString());
        }
    ...
    private static class Templates {
         ...
         */
        public static native TemplateInstance listPullRequestsTargetingBranch(String repo, String branch);
